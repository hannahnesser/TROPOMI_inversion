...
filename = './canada-emission/Area_Global.nc'
...
for isec = 0, N_GCsector-1 do begin
	...
	filesec=['1B2ci']
        varsec=['1B2ci1 (oil production)','1B2ci1 (oil refining)',$
	...
	varsec=['1B2cii1 (oil production)','1B2cii1 (oil refining)',$
        ...
        filesec=['1B2ci']
        varsec=['1B2ci2 (distribution)','1B2ci2 (gas processing)',$
                '1B2ci2 (gas production)','1B2ci2 (transmission)']
        endif

        if thissec eq 'gas3' then begin
        filesec=['1B2cii']
        varsec=['1B2cii2 (gas processing)',$
                '1B2cii2 (gas production)','1B2cii2 (transmission)']
        endif
	...
	for ifile = 0,nsecfile-1 do begin
		filename = './canada-emission/can_emis_'+filesec[ifile]+'_2018.nc'
		Cdfid=NCDF_OPEN(filename)
...
if (donc) then begin
        ...
        filename='./postncfile/CAN_Tia2020_'+thissec+'_2018.nc'
