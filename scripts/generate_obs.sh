...
jid2=$(sbatch --dependency=afterok:${jid1##* } --array=1-12 apply_TROPOMI_operator.sh ${CODE_DIR} ${TROPOMI_DIR} ${PRIOR_DIR} ${PRIOR_DIR} ${JACOBIAN})
# jid2=$(sbatch --array=1-12 apply_TROPOMI_operator.sh ${TROPOMI_DIR} ${PRIOR_DIR} ${PRIOR_DIR})

# Analyze the output
sbatch --dependency=afterok:${jid2##* } run_generate_obs.sh ${CODE_DIR} ${PRIOR_DIR} ${OUTPUT_DIR}
