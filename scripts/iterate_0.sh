...
jid3=$(sbatch --dependency=afterok:${jid2##* } --array=1-12 generate_pph.sh "0" ${SHORT_TERM_DATA_DIR} ${CODE_DIR})
# sbatch --array=1-12 generate_pph.sh "0" ${SHORT_TERM_DATA_DIR} ${CODE_DIR}
...
# sbatch generate_evecs.sh "0" ${NUM_EVECS} ${SHORT_TERM_DATA_DIR} ${LONG_TERM_DATA_DIR} ${CODE_DIR}
