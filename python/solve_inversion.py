if __name__ == '__main__':
    ...
    if optimize_rf:
        ...
        rfs = [1e-3, 1e-2, 5e-2, 1e-1, 5e-1, 1]
        sas = [0.1, 0.25, 0.5, 0.75, 1]
        ja = np.zeros((len(rfs), len(sas)))
        jo = np.zeros((len(rfs), len(sas)))
        n = np.zeros((len(rfs), len(sas)))
        for i, rf_i in enumerate(rfs):
            for j, sa_i in enumerate(sas):
                ...
                pre_xhat_i = sa_i**2*rf_i*copy.deepcopy(pre_xhat)/0.25
                ...
                ja[i, j] = (((xhat - np.ones(xhat.shape))**2
                             /sa.reshape(-1,)).sum())
        ...
        # Save the result
        np.save(f'{data_dir}/iteration{niter}/ja{niter}_long.npy', ja)
        np.save(f'{data_dir}/iteration{niter}/jo{niter}_long.npy', jo)
        np.save(f'{data_dir}/iteration{niter}/n_functional.npy', n)

    ## ---------------------------------------------------------------------##
    ## Optimize the magnitude of the prior errors via cost function analysis
    ## (This seems like a suboptimal strategy)
    ## ---------------------------------------------------------------------##
    if optimize_sa:
        # To calculate the cost function for the observational term,
        # we need to load the true observations and the constant term
        ...
        sas = [0.1, 0.25, 0.5, 0.75, 1]
        ja_sa = np.zeros(len(sas))
        jo_sa = np.zeros(len(sas))
        n = np.zeros(len(sas))
        for i, sa_i in enumerate(sas):
            print(i, sa_i)
            # Scale the relevant terms by RF
            evals_h_i = sa_i*copy.deepcopy(evals_h_sub)
            pre_xhat_i = sa_i*copy.deepcopy(pre_xhat)
            ...
            # jo[i] = ((y.values - yhat)**2/(so.reshape(-1,)/sa_i)).sum()
