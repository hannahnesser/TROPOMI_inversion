if __name__ == '__main__':
    ...
    # Save the result
    np.save(f'{data_dir}/iteration{niter}/a{niter}{suffix}.npy', a)
    np.save(f'{data_dir}/iteration{niter}/dofs{niter}{suffix}.npy', dofs)
    # np.save(f'{data_dir}/iteration{niter}/xhat/xhat{niter}{suffix}.npy', xhat)
    np.save(f'{data_dir}/iteration{niter}/xhat_fr{niter}{suffix}.npy', 
            xhat_fr)
    np.save(f'{data_dir}/iteration{niter}/shat_kpi{niter}{suffix}.npy', 
            shat)
    ...
    for country, mask in masks.items():
        ...
        r_red.to_csv(f'{data_dir}/iteration{niter}/countries/r{niter}{suffix}_{country.lower()}.csv', header=True, index=True)
        a_red.to_csv(f'{data_dir}/iteration{niter}/countries/a{niter}{suffix}_{country.lower()}.csv', header=True, index=True)
    ...
    for label, mask in sub_masks.items():
        ...
        r_red.to_csv(f'{data_dir}/iteration{niter}/{label.lower()}/r{niter}{suffix}_{label.lower()}.csv', header=True, index=True)
        a_red.to_csv(f'{data_dir}/iteration{niter}/{label.lower()}/a{niter}{suffix}_{label.lower()}.csv', header=True, index=True)
    ...
    for key, city in cities.items():
        ...
        _, _, r_red, a_red = ip.source_attribution(w_c, xhat_fr, shat, a)
        r_red.to_csv(f'{data_dir}/iteration{niter}/urban/r{niter}{suffix}_urban_areas_{key}.csv', header=True, index=True)
        a_red.to_csv(f'{data_dir}/iteration{niter}/urban/a{niter}{suffix}_urban_areas_{key}.csv', header=True, index=True)
