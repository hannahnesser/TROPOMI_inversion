...
city_studies = {
    ...
    'Jeong et al. (2016)' : {'Los Angeles' : [380, 79, 110],
                             'San Francisco' : [245, 86, 95]}, # June 2013 - May 2014 https://agupubs.onlinelibrary.wiley.com/doi/pdfdirect/10.1002/2016JD025404
    ...
    'Guha et al. (2020)' : {'San Francisco' : [226, 40, 40]}, # 88% increase over inventory, 2015 - 2019 measurements https://pubs-acs-org.ezp-prod1.hul.harvard.edu/doi/pdf/10.1021/acs.est.0c01212
    ...
    r'Plant et al. (2019) (CO$_2$)' : {'Washington' : [340, 134, 275],
                                       'Philadelphia' : [394, 157, 134],
                                       'New York' : [1187, 437, 477],
                                       'Boston' : [210, 54, 57]}, # Aircraft campaign measurement of CH4/CO and CH4/CO2 ratios
...
#         color = sf_cmap(div_norm(summ_c.loc[shape.record[3]]['xhat_mean']))
...
corr_c = dc(summ_c)
corr_c = corr_c[corr_c['dofs_mean'] >= 0.1]
...
for i, q in enumerate(['diff', 'xhat']):
    ...
    ax[1, 0].errorbar(corr_c['pop_2010']/corr_c['pop_2000'] - 1, 
                      ...
                      yerr=np.array(corr_c[[f'{q}_min', f'{q}_max']]).T,
                      elinewidth=0.5)
    ax[1, 0] = fp.add_labels(ax[1, 0], r'Relative population change 2012-2019',
...
for i, (l, e) in enumerate(emis.items()):
    for j in range(2):
        ax[j].barh(ys - 0.175, summ_c[f'prior_{e}'], left=left_prior, 
                   height=0.3, color=cc[i], label=f'{l}')
    left_prior += summ_c[f'prior_{e}']

# Posterior
for j in range(2):
    ax[j].barh(ys + 0.175, summ_c['post_mean'],
    ...
    for cs_name, cs in city_studies.items():
        for c_name, cd in cs.items():
            if c_name in city_names:
                ...
                ax[j].errorbar(cd[0], y + 1, xerr=np.array(cd[1:])[:, None], 
                               fmt=formats[i % 9], markersize=3, 
...
ax[0].text((summ_c['post_mean'] + summ_c['post_max'])[0] + 10, ys[0] + 0.075,
           'Posterior',
           ha='left', va='top', 
           fontsize=config.TICK_FONTSIZE)

# Plot emissions per capita
ax[2] = fp.add_title(ax[2], 
                     'Per capita urban emissions\nin largest CONUS cities', 
                     fontsize=config.TITLE_FONTSIZE)
left_prior = np.zeros(nc)
left_post = np.zeros(nc)
for i, (l, e) in enumerate(emis.items()):
    ax[2].barh(ys - 0.175, summ_c[f'prior_{e}']/summ_c['pop_2010']*1e6, 
              left=left_prior, height=0.3, color=cc[i], label=l)
    left_prior += summ_c[f'prior_{e}']/summ_c['pop_2010']*1e6
ax[2].barh(ys + 0.175, summ_c[f'post_mean']/summ_c['pop_2010']*1e6, 
...
ax[2].invert_yaxis()
ax[2] = fp.add_labels(ax[2], 
                      r'Emissions per capita''\n(kg person'r'$^{-1}$ a$^{-1}$'')', 
                      '',
                      fontsize=config.TICK_FONTSIZE, 
                      labelsize=config.TICK_FONTSIZE, labelpad=10)

ax[2].tick_params(axis='both', labelsize=config.TICK_FONTSIZE)
ax[2].axvline(0, ls=':', lw=0.5, color='0.5')
ax[2].set_xlim(0, 75)
for i in range(3):
    ax[2].axvline((i + 1)*20, color='0.75', lw=0.5, zorder=-10)
...
# print(len(labels))
# reorder = np.arange(24).reshape(4, 6).T.flatten()[:len(labels)]
# print(reorder)
...
ax[2] = fp.add_legend(ax[2], handles=handles, labels=labels, ncol=6,
