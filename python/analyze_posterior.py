...
for country, mask in masks.items():
    # Posterior terms
    post_w37_c = (dc(w['w37_edf'])*mask) @ xhat[w37_cols]
    ...
    post_sub_w37_c = (dc(w['w37_edf'])*mask) @ xhat_sub[w37_cols]
    ...
    # Prior terms
    prior_w37_c = (dc(w['w37_edf'])*mask) @ np.ones(xhat[w37_cols].shape)
    ...
    prior_sub_w37_c = (dc(w['w37_edf'])*mask) @ xa_sub[w37_cols]
...
fig, axis = fp.get_figax(aspect=1.5, cols=3, sharey=True)
...
for c, e in summ_c.items():
    ax = axis[j]
    ax = fp.add_title(ax, f'{c}', fontsize=config.TITLE_FONTSIZE)

    # Plot the total bar (all emissions, unoptimized and optimized)
    ax.barh(ys - 0.175, e['mean'][prior_rows], 
            height=0.3, color=cc, alpha=0.3, label=emis.keys())
    ...
    # ax.barh(ys + 0.35, e['lu'], width=0.1, 
    #       color=fp.color(2*j+2), alpha=0.3, label='Lu et al. (2022)')
...
# Add labels
for i in range(3):
    axis[i].set_yticks(ys)
    axis[i].set_yticklabels('', ha='right',
                            fontsize=config.TICK_FONTSIZE)
    axis[i].set_ylim(0, 8)
    axis[i].invert_yaxis()
    axis[i] = fp.add_labels(axis[i], r'Emissions (Tg a$^{-1}$)', '',
                            fontsize=config.TICK_FONTSIZE,
                            labelsize=config.TICK_FONTSIZE,
                            labelpad=10)
    for j in range(6):
        axis[i].axhline((j + 1) + 0.5, color='0.75', lw=0.5, zorder=-10)

axis[0].set_yticklabels(list(emis.keys()), ha='right',
                        fontsize=config.TICK_FONTSIZE)

# Add prior/posterior labels 
left = summ_c['Canada']['mean'] + summ_c['Canada']['max']
axis[0].text(summ_c['Canada']['mean']['prior_livestock'] + 0.25, ys[0] + 0.05,
             'Prior', ha='left', va='bottom', fontsize=config.TICK_FONTSIZE)
axis[0].text(left['post_livestock'] + 0.25, ys[0] + 0.075, 'Posterior', 
             ha='left', va='top', fontsize=config.TICK_FONTSIZE)

# Add grid lines
for j in range(3):
    axis[0].axvline(2.5*(j + 1), color='0.75', lw=0.5, zorder=-10)

for j in range(2):
    axis[1].axvline(5*(j + 1), color='0.75', lw=0.5, zorder=-10)

for j in range(2):
    axis[2].axvline((j + 1), color='0.75', lw=0.5, zorder=-10)

# Add legend
custom_patches = [patch(facecolor='0.5', alpha=0.3),
